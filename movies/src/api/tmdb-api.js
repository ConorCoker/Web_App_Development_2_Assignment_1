export const getMovies = ({queryKey}) => {
  const [, sortByPart] = queryKey;
  const { sortBy } = sortByPart;  
  return fetch(
    `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-US&include_adult=true&include_video=false&page=1&sort_by=${sortBy}`
  ).then((response) => {
    if (!response.ok) {
